...
Usage: $(basename "$0") [OPTION]...
 [Required]
  ...
  -c <dcap repo path>               The path to the dcap repo
 [Optional]
...
process_args() {
    while getopts "hu:r:b:o:p:k:s:l:v:d:i:c:" option; do
        case "$option" in
...
check_args_env() {
    ...
    if [[ -z ${DCAP_REPO_LOCAL} ]]; then
        error "DCAP package repo is required, please provide by '-c' ."
        # shellcheck disable=SC2317
        exit 1
    fi
    ...
    if [[ -n ${DCAP_REPO_LOCAL} ]]; then
        if [[ "${DCAP_REPO_LOCAL: -1}" == "/" ]]; then
            DCAP_REPO_LOCAL=${DCAP_REPO_LOCAL%?}
        fi
        DCAP_REPO_NAME=${DCAP_REPO_LOCAL##*/}
    fi
